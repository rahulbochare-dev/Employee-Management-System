...
import {
  ...
  getPendingLeave,
  getLastWeeksLeaves,
  getMostEmployeeCountry,
  getTotalPayrollThisMonth,
  getEmployeeGenderRatioPercent,
...
const useDashboardStore = create((set, get) => ({
  ...
  getKPIData: async () => {
    ...
    try {
      ...
      set({
        mostEmployeeCountry: mostEmployeeCountryResponse.data.data,
        loading: false,
      });

      const totalPayrollThisMonthResponse = await getTotalPayrollThisMonth();
      set({
        totalPayrollThisMonth: totalPayrollThisMonthResponse.data.data,
        loading: false,
      });

      const employeeGenderRatioPercentResponse =
        await getEmployeeGenderRatioPercent();
      set({
        employeeGenderRatioPercent:
          employeeGenderRatioPercentResponse.data.data,
        loading: false,
      });

      const averageEmployeeAgeResponse = await getAverageEmployeeAge();
      set({
        averageEmployeeAge: averageEmployeeAgeResponse.data.data[0],
        loading: false,
      });

      const newJoinesByMonthResponse = await getNewJoinesByMonth();
      set({
        newJoinesByMonth: newJoinesByMonthResponse.data.data,
        loading: false,
        dataFetched: true,
      });
    } catch (err) {
