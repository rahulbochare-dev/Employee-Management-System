...
const LeaveDetailsEmployee = ({ leaveDetails, cb }) => {
  ...
  useEffect(() => {
    const callApi = async () => {
      if (id && !leaveDetails) {
        await getLeaveDetails(id);
      }
    };
    callApi();
  }, []);

  const leave = leaveDetails || myLeaveDetails;
    ...
    <div className="w-304.5 h-155.5 bg-white rounded-2xl px-8.5 pt-6 overflow-hidden transition-all">
      ...
      <div className="w-full h-127.5 flex">
        <div className="w-107.5 h-full pr-8 border-r border-[#dddddd]">
          ...
          <div className="w-full h-28 flex items-center gap-5">
            ...
            <div className="flex flex-col">
              ...
              <h3 className="text-[1rem] text-[#7a7a7a] font-medium mt-2 leading-none">
                ID: {leave?.employee.empID} • {leave?.employee.email}
              </h3>
            </div>
          </div>
          ...
          <div className="w-full grid grid-cols-2 gap-y-10 gap-x-20 mt-3 pl-10">
            ...
            <div className="flex flex-col">
              <h3 className="text-[1.25rem] text-[#666666] font-medium">To:</h3>
              <h2 className="text-[1.25rem] text-black font-medium mt-1.5">
        ...
        <div className="flex-1 pl-9 flex flex-col">
          <h2 className="text-[1.5rem] font-medium text-black">Description:</h2>
          ...
          <div className="w-full flex justify-end items-center gap-6 mt-8">
            <button
              ...
              value={"Rejected"}
              name="status"
              className="w-33.25 h-10.25 rounded-[0.875rem] bg-[#f58484] text-[1rem] font-medium text-black transition-all hover:bg-[#ef7575] active:bg-[#e76767]">
              Delete
            </button>
            <button
              onClick={cb}
              className="w-33.25 h-10.25 rounded-[0.875rem] border border-[#d3d3d3] bg-[#fafafa] text-[1rem] font-medium text-black transition-all hover:bg-[#f2f2f2] active:bg-[#ebebeb]">
              Close
            </button>
          </div>
        </div>
